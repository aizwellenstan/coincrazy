// Version 1.2.3 - Copyright 2012,2013 e frontier, Inc. All Rights Reserved.
...
class ShadeToUnityPostprocessor extends AssetPostprocessor {
    function GetVersion () { return 1202; }
...
//    }
    static function GetShadePath () {
        if (Application.platform == RuntimePlatform.OSXEditor) {
            var appPath =  "/Applications/Shade 3D ver.14E.app/Contents/MacOS/xshade";
            if (CheckShade(appPath, false)) return appPath;
            appPath =  "/Applications/Shade 3D Basic ver.14.app/Contents/MacOS/xshade";
            if (CheckShade(appPath, false)) return appPath;
            appPath =  "/Applications/Shade 3D ver.14.app/Contents/MacOS/xshade";
            if (CheckShade(appPath, false)) return appPath;
            appPath =  "/Applications/Shade 3D for Unity.app/Contents/MacOS/xshade";
            if (CheckShade(appPath, false)) return appPath;
        ...
        if (Application.platform == RuntimePlatform.WindowsEditor) {
            var regKey = Microsoft.Win32.Registry.ClassesRoot.OpenSubKey(".shd");
            var exe_path = "";
            if (regKey) {
                ...
                if (regKey) {
                    ...
                    if (exe_path && CheckShade3D(exe_path, false)) return exe_path;
                }
            }
            regKey = Microsoft.Win32.Registry.LocalMachine.OpenSubKey("SOFTWARE\\e-frontier, Inc.\\Shade 3D for Unity");
            if (regKey) {
            ...
            if (regKey) {
                fileType = regKey.GetValue("") as String;
                regKey.Close();
                regKey = Microsoft.Win32.Registry.ClassesRoot.OpenSubKey(fileType + "\\shell\\open\\command");
                if (regKey) {
                    command = regKey.GetValue("") as String;
                    regKey.Close();
                    exe_path = command.Replace("\"", "").Replace("%1", "");
                    if (exe_path && CheckShade(exe_path, false)) return exe_path;
                }
    ...
    static function CheckShade3D (shadePath:String, showLog:boolean) {
        ...
        if (File.Exists(shadePath)) {
            ...
            // Shade 3D ver.14.
            var match = Regex.Match(output, "Shade 3D (.*) ver.(([0-9]+[.])?([0-9]+[.])?[0-9]+)( DEMO)?");
            if (match.Success) {
                var version = match.Groups[2].Value;
                var grade = match.Groups[1].Value;
                var demo = match.Groups[5].Value;
    ...
    static function CheckShade (shadePath:String, showLog:boolean) {
        ...
        if (File.Exists(shadePath)) {
            ...
            var match = Regex.Match(output, "Shade 3D (.*) ver.(([0-9]+[.])?([0-9]+[.])?[0-9]+)( DEMO)?");
            if (match.Success) {
                //UnityEngine.Debug.Log("Shade 3D Loader: " + match.Groups[5].Value);
                var version = match.Groups[2].Value;
                var grade = match.Groups[1].Value;
                var demo = match.Groups[5].Value;
                //UnityEngine.Debug.Log("Shade 3D Loader: " + version);
			...
			// Shade 13.
            match = Regex.Match(output, "Shade (([0-9]+[.])?([0-9]+[.])?[0-9]+) (.*)");
            if (match.Success) {
                version = match.Groups[1].Value;
                grade = match.Groups[4].Value;
                //UnityEngine.Debug.Log("Shade 3D Loader: " + version);
                //UnityEngine.Debug.Log("Shade 3D Loader: " + grade);
                match = Regex.Match(output, "Build version ([0-9]+)");
                if (match.Success) {
                    buildnumber = parseInt(match.Groups[1].Value as String);
        ...
        for (var str in importedAssets) {
            if (Path.GetExtension(str) == ".shd") {
                ...
                if (CheckShade(shadePath, true)) {
                    //UnityEngine.Debug.Log(shadePath);
